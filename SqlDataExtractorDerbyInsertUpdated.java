...
public class SqlDataExtractorDerbyInsertUpdated {
					...
					while (rsSub.next()) {
						...
						for (int i = 1; i <= rowCount; i++) {
							...
							if (isPrimitive(type)) {
								...
								if (i == rowCount) {
									if (rsSub.getString(i) != null) {
										sb.append("'" + rsSub.getString(i).trim().replaceAll("'", "''").replaceAll("xA0", " ").replaceAll("x92", "\"").replaceAll("x93", "\"").replaceAll("x94", "\"").replaceAll("x95", "\"") + "'");
									} else {
										sb.append("" + null);
									}
								} else {
